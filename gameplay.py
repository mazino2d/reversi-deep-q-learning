...
def playGame(p1, p2, verbose=False, t=128):
    ...
    if verbose:
        printBoard(board)
        print(
            "START: Clock remaining: %s=%f, %s=%f"
            % (currColor, p1time, nextColor, p2time)
        )
    while not gameOver(board):
        ...
        if verbose:
            printBoard(board)
            print(
                "Clock remaining: %s=%f, %s=%f" % (currColor, p1time, nextColor, p2time)
            )
...
if __name__ == "__main__":
    ...
    if reversed != "R":
        p1 = nextMove  # pylint:disable=undefined-variable
    else:
        p1 = nextMoveR
    ...
    elif ((res[0] > res[1]) and reversed != "R") or (
        (res[0] < res[1]) and reversed == "R"
    ):
        print("%s Wins %s Loses (%d to %d)" % (args[0], args[1], res[0], res[1]))
    elif ((res[0] < res[1]) and reversed != "R") or (
        (res[0] > res[1]) and reversed == "R"
